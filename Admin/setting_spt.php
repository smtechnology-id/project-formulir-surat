...
$sql = mysqli_query($conn, "SELECT * from tb_dasar_spt ORDER BY no_urut ASC");
if (isset($_POST['inputDasar'])) {
    $textDasar = $_POST['textDasar'];
    $noUrut = $_POST['noUrut'];
    
    $sqlInputDasar = mysqli_query($conn, "INSERT INTO tb_dasar_spt values ('', '$textDasar', $noUrut)");
    if ($sqlInputDasar) {
        echo '<script>alert("Berhasil Menambahkan Item")</script>';
        header("location: setting_spt.php");
    }
}

if (isset($_GET['idDelete'])) {
    $idDasar = $_GET['idDelete'];

    $sqlDelete = mysqli_query($conn, "DELETE FROM tb_dasar_spt where id_dasar_spt = $idDasar");
    header("location: setting_spt.php");
}
                    ...
                    <ul>
                        ...
                        <li class="has_sub">
                            ...
                            <ul class="list-unstyled">
                                ...
                                <li><a href="pegawai_jabatan_jenis.php">Data Jabatan Jenis</a></li>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <h4 class="mt-0 header-title">Setting Dasar Surat Perintah tugas (SPT)</h4>
                                        <button type="button" class="btn btn-primary waves-effect waves-light my-3" data-toggle="modal" data-target="#myModal"><i class="fa fa-plus" aria-hidden="true"></i></button>
                                        <a href="surat_perjalanan_dinas.php" class="btn btn-light waves-effect waves-light my-3"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
                                        <table id="tablePegawai" class="table mb-0" id="my-table">
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                    <tr>
                                                        <td><?= $fetchSql['no_urut'] ?></td>
                                                        <td><?= $fetchSql['item_dasar'] ?></td>
                                                        <td>
                                                            
                                                            <a href="setting_spt.php?idDelete=<?php echo $fetchSql['id_dasar_spt'] ?>" class="btn btn-sm btn-danger"><i class="fa fa-trash">
            ...
            <div class="modal-content">
                ...
                <form method="post" action="">
                    <div class="modal-body">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="mt-0 header-title">Masukkan Text Dasar SPT</h4>

                                <div class="form-group row">
                                    <label for="example-text-input" class="col-sm-2 col-form-label">No Urut</label>
                                    <div class="col-sm-10">
                                        <select class="form-control" name="noUrut" required>
                                ...
                                <div class="form-group row">
                                    <label for="example-text-input" class="col-sm-2 col-form-label">Text Dasar</label>
                                    <div class="col-sm-10">
                                        <textarea name="textDasar" id="" class="form-control" cols="30" rows="10"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="submit" name="inputDasar" value="Input" class="btn btn-primary">
                    </div>
                </form>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
